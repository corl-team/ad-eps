...
def split_info_debug(eval_info, debug_info, train_goals, test_goals):
    ...
    for i, (k, v) in enumerate(eval_info.items()):
        if list(k) in train_goals:
            ...
            if len(debug_train["states"]) == 0:
                debug_train["states"] = debug_info["states"][:, i]
                debug_train["actions"] = debug_info["actions"][:, i]
                debug_train["goal"] = debug_info["goal"][i]
        elif list(k) in test_goals:
            eval_info_test[k] = v
            if len(debug_test["states"]) == 0:
                debug_test["states"] = debug_info["states"][:, i]
                debug_test["actions"] = debug_info["actions"][:, i]
                debug_test["goal"] = debug_info["goal"][i]
        else:
            print("wtf!!")
...
def train(config: TrainConfig):
    ...
    max_optimal_return = calculate_eps_reward(train_goals, config.env_name, eps=0)
    ...
    global_step = 0
    epoch = 0
    pbar = tqdm(total=config.num_updates, position=0)
        ...
        for states, actions, rewards in dataloader:
            ...
            if global_step % config.eval_every == 0:
                ...
                eval_info, debug_info = evaluate_in_context(
                    env_name=config.env_name,
                    model=model,
                    goals=train_goals.tolist() + test_goals.tolist(),
                ...
                (
                    ...
                    debug_train,
                    debug_test,
                ) = split_info_debug(eval_info, debug_info, train_goals, test_goals)

                pic_name_train = draw_sample_eff_graph(
                    eval_info_train,
                    ylim=[-0.3, 20],
                    name=f"train; max_perf: {config.max_perf}",
                ...
                pic_name_test = draw_sample_eff_graph(
                    eval_info_test,
                    ylim=[-0.3, 20],
                    name=f"test; max_perf: {config.max_perf}",
                    max_return=max_optimal_return,
                    max_return_eps=max_perf_return,
                )
